...
using SharpNeat;
//using SharpNeat.NeuralNetwork;
//using SharpNeat.NeuralNetwork.Xml;
//using SharpNeat.NetworkVisualization;

//using SharpNeat.NeatGenome;
//using SharpNeat.NeatGenome.Xml;
...
namespace NetworkViewer
{
	...
	public class Form1 : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
            ...
            // 
            this.mnuFileOpenGenomeXml.Index = 0;
        ...
        private void mnuFileOpenGenomeXml_Click(object sender, EventArgs e)
        {
            ...
            // Find the Network element (buried in outer content).
            XmlElement networkElem = (XmlElement)doc.SelectSingleNode("/Root/Networks/Network");

			//----- Read the genome structure from the XmlDocument.
			NeatGenome genome = null;
			try
			{
                genome = NeatGenomeXmlIO.LoadGenome(networkElem, false);
			}
			catch(Exception ex)
			{
            ...
            // Apparently we need a NeatGenomeFactory to be set-up.
            genome.GenomeFactory = new NeatGenomeFactory(genome.InputNodeCount, genome.OutputNodeCount); 



            IOGraph graph = _graphFactory.CreateGraph(genome);
