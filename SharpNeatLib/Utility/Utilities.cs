...
namespace SharpNeat.Utility
{
    ...
    public static class Utilities
    {
        ...
        {
            Debug.Assert(valueList.Count != 0 && IsSorted(valueList),
                "CalculateMedian() requires non-zero length sorted list of values.");
        ...
        /// <summary>
        ///     Carries out a primality test on the given number.
        /// </summary>
        /// <param name="number">The number to check for primality.</param>
        /// <returns>Whether or not the given number is prime.</returns>
        public static bool IsPrime(int number)
        {
            ...
            // Compute square root and ensure that this is a whole number (reduces number of iterative comparisons)
            var N = (long) (Math.Sqrt(number) + 0.5);

            // Test for odd divisors up to square root of original number
            for (var i = 3; i <= N; i += 2)
            {
                if (number % i == 0)
                    return false;
        ...
        public static FrequencyDistributionData CalculateDistribution(double[] valArr, int categoryCount)
        {
            ...
            if (0.0 == range)
            {
                return new FrequencyDistributionData(min, max, 0.0, new[] {valArr.Length});
            }
        ...
        public static double MagnifyFitnessRange(double x, double metricThreshold, double metricMax,
            double fitnessThreshold, double fitnessMax)
        {
            if (x < 0.0)
            {
                x = 0.0;
            ...
            {
                return ((x - metricThreshold) / (metricMax - metricThreshold) * (fitnessMax - fitnessThreshold)) +
                       fitnessThreshold;
            }
